...
class VideoDCD(nn.Module):
    ...
    def __init__(self, x_h=64, x_w=64, filter_size=128, j=16, nchannel=1, bn=False):
        super(VideoDCD, self).__init__()

        # self.batch_size = batch_size  # ! do not write batch_size as fixed
    ...
    def forward(self, inputs1, inputs2=None):
        ...
        sample_size = inputs1.size(0)
        x = inputs1.reshape(sample_size * time_steps, self.nchannel, self.x_height, self.x_width)
        x = self.conv_net(x)
        x = x.reshape(sample_size, time_steps, -1)
        ...
        # print("\tIn Model: input size", inputs1.size(),
        #       "output size", x1.size())

        if inputs2 is not None:
            time_steps = inputs2.shape[1]
            sample_size = inputs2.shape[0]
            x = inputs2.reshape(sample_size * time_steps, self.nchannel, self.x_height, self.x_width)
            x = self.conv_net(x)
            x = x.reshape(sample_size, time_steps, -1)
            # first output dimension is the sequence of h_t.
            # second output is h_T and c_T(last cell state at t=T).
...
class VideoDCG(nn.Module):
    ...
    def __init__(self, time_steps=32, x_h=64, x_w=64, filter_size=32, state_size=32, nchannel=1, z_dim=25,
    ...
    def forward(self, z, y):
        ...
        z = z.reshape(sample_size, self.time_steps, self.n_noise_t)
        y = y[:, None, :].expand(sample_size, self.time_steps, self.n_noise_y)
        x = torch.cat([z, y], -1)
        ...
        if self.bn:
            x = self.lstmbn2(x)
        x = x.permute(0, 2, 1)
        x = x.reshape(sample_size * self.time_steps, -1)
        x = self.dense_net(x)
        x = x.reshape(sample_size * self.time_steps, self.filter_size * 4, 8, 8)
        x = self.deconv_net(x)
        # x = self.final1(x)
        # x2 = self.final2(x)
        x = x.reshape(sample_size, self.time_steps, self.nchannel, self.x_height, self.x_width)
